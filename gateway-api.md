...
| 渠道 | 说明 | 延迟 | 可靠性 |
|------|------|------|--------|
| **HTTP 回调** | POST 到下单时传入的 `notify_url` | 秒级 | 高（最多重试 10 次，指数退避） |
| **WebSocket 推送** | 实时推送到合作伙伴的 WS 连接 | 毫秒级 | 依赖连接状态 |

**回调 URL 配置（合作伙伴在微信/支付宝商户平台设置）：**

| 支付平台 | 回调地址 | 说明 |
|---------|---------|------|
| 微信支付 | `https://www.sioyun.com/api/v1/partner/wechat/applyment/notify` | 支付回调与进件回调共用同一入口，网关内部按 `out_trade_no` 自动区分 |
| 支付宝 | `https://www.sioyun.com/api/pay/alipay/notify` | 同上 |

> **注意**：支付平台侧只需填写网关的统一回调地址，合作伙伴自己的 `notify_url` 在下单时传入即可。网关收到支付平台回调后，会自动转发到合作伙伴的 `notify_url`。

**处理流程：**

```
1. 用户支付成功 → 微信/支付宝回调网关
2. 网关验签 → 解密资源 → 按 out_trade_no 查 orders
3. 命中支付订单：
   ├── 更新 orders.status = SUCCESS、pay_amount、pay_time
   ├── 构造 CallbackPayload → POST 到合作伙伴 notify_url
   ├── 同时 WebSocket 推送 payment.success 事件
   └── 记录 CallbackLog（失败自动重试，最多 10 次）
4. 未命中支付订单：
   └── 交进件分支处理（微信/支付宝进件回调）
```

**HTTP 回调请求格式、签名验签、事件类型定义、重试策略等详见 [第 7 章 回调通知接口](#7-回调通知接口)。**

**回调管理 API（自查投递状态）：**

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/gateway/v1/notify/status/:gateway_trade_no` | 查询某次回调投递状态 |
| GET | `/api/gateway/v1/notify/logs?out_trade_no=xxx` | 按订单号查投递日志 |
